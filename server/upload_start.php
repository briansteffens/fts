	...
	Request:
		{
			...
			"chunk_hashes": [[string]],	// Each chunk's individual hash (sha256) in an array
	...
	Response:
		{
			...
			"next_chunk_index_hint": [int], // A missing chunk that could be uploaded
...
$q = $db->prepare("insert into files (id, file_size, chunk_size, file_hash, file_name, content_type, date_started) values (?,?,?,?,?,?,now());");
$q->bind_param("siisss", $file_id, $digest->file_size, $digest->chunk_size, $digest->file_hash, $digest->file_name, $digest->content_type);
$q->execute();
$q->close();

for ($i = 0; $i < sizeof($digest->chunk_hashes); $i++) {
	$q = $db->prepare("insert into chunks (file_id, chunk_index, chunk_hash) values (?,?,?);");
